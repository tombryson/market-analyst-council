...
class SourceResolver:
    ...
    def resolve(self, event: AnnouncementEvent) -> AnnouncementPacket:
        ...
        return AnnouncementPacket(
            ...
            exchange=exchange,
            title=title,
            published_at_utc=str(event.received_at_utc or "").strip(),
    ...
    @staticmethod
    def _infer_source_type(event: AnnouncementEvent, source_url: str) -> str:
        sender = str(event.sender or "").strip().lower()
        host = urlparse(str(source_url or "").strip()).netloc.lower()
        if host == "announcements.asx.com.au" or host.endswith("asx.com.au") or "asx" in sender:
