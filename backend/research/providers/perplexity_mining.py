"""Segmented Perplexity mining supplementary-facts workflow."""
...
from ..mining_supplementary import (
    SEGMENT_DEFINITIONS,
...
class PerplexityMiningSupplementaryEnricher:
    """Reusable segmented mining supplementary-facts orchestration."""
    ...
    async def gather(
        ...
        *,
        ...
        context = resolve_mining_enricher_context(
        ...
        commodity_name = str(context.get("commodity") or "").strip().lower()
        ...
        self.provider._log(
            f"mining_enricher start ticker={exchange_code}:{ticker_symbol} preset={active_preset} repair_preset={active_repair_preset} repairs={allow_repairs}"
        )
        ...
        if allow_repairs and missing_after_adjudication:
            ...
            for repair_spec in repair_specs:
                ...
                try:
                    ...
                    self.provider._log(
                        f"mining_enricher repair_fail_open ticker={exchange_code}:{ticker_symbol} repair={repair_spec['name']} err={type(exc).__name__}: {exc}"
                    )
        ...
        if PERPLEXITY_MINING_CONTAMINATION_CHECKER_ENABLED:
            try:
                ...
                self.provider._log(
                    f"mining_enricher contamination_fail_open ticker={exchange_code}:{ticker_symbol} err={type(exc).__name__}: {exc}"
                )

        self.provider._log(
            f"mining_enricher done ticker={exchange_code}:{ticker_symbol} missing_after_merge={len(missing_after_merge)} missing_after_final={len(missing_or_not_found_items(final_json))} elapsed={perf_counter() - run_start:.1f}s"
        )

        return {
            "provider": self.provider.name,
            "workflow": "perplexity_mining_supplementary_v1",
            "company": company_name,
        ...
        *,
        ...
        try:
            ...
            if _is_invalid_request(exc):
                ...
                self.provider._log(
                    f"mining_enricher retry stage={stage_name} reason=invalid_request fallback_reasoning=low"
                )
            ...
            if str(preset or "").strip().lower() == "advanced-deep-research":
                ...
                self.provider._log(
                    f"mining_enricher retry stage={stage_name} reason=timeout fallback_preset=deep-research"
                )
                data = await _post(retry_payload)
                payload = retry_payload
            else:
        ...
        if not isinstance(parsed_json, dict):
            ...
            self.provider._log(
                f"mining_enricher retry stage={stage_name} reason=parse_error err={parse_error}"
            )
            data = await _post(retry_payload)
            payload = retry_payload
            raw_text = self.provider._extract_content(data)
        ...
        self.provider._log(
            f"mining_enricher stage_done stage={stage_name} model={data.get('model') or payload.get('model')} chars={len(raw_text)} elapsed={perf_counter() - request_start:.1f}s"
        )
        return {
            "stage_name": stage_name,
    ...
    async def _repair_json_with_openrouter(
        ...
        *,
        ...
        self.provider._log(
            f"mining_enricher json_repair stage={stage_name} model={CHAIRMAN_JSONIFIER_MODEL}"
        )
        repair_prompt = (
        ...
        if not response:
            return None
        repaired_text = str(response.get("content") or "").strip()
        repaired_json, _ = extract_json_payload(repaired_text)
